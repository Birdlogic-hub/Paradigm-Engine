...
lines = SC_get("Event Log").entry.split("\n");
H.assert(lines.length === 16 && /event 11/.test(lines[1]), "window is 20 (v0.4.4): all 15 events retained, newest on top");
...
// --- v0.4.2: the two banners + the live migration ------------------------------------
const cfgT = SC_config("Banner Config", { KNOB: 1 });
H.assert(SC_get("Banner Config").type === "Settings", "config cards default to the Settings banner");
SC_get("Banner Config").type = "ParadigmEngine";          // a card from before the split
SC_config("Banner Config", { KNOB: 1 });
H.assert(SC_get("Banner Config").type === "Settings", "config heal on the FOUND path: legacy cards re-type live");
SC_reportEnsure();
SC_get("Event Log").type = "ParadigmEngine";
SC_reportEnsure();
H.assert(SC_get("Event Log").type === "Gameplay", "the Event Log re-types to Gameplay (it is read, not edited)");

// --- v0.4.3: description heal — engine docs reach live cards --------------------------
SC_config("Banner Config", { KNOB: 1 }, { description: "docs v1" });
H.assert(SC_get("Banner Config").description === "docs v1", "description heals onto an existing card");
SC_config("Banner Config", { KNOB: 1 }, { description: "docs v2" });
H.assert(SC_get("Banner Config").description === "docs v2", "module upgrades rewrite stale NOTES");
SC_config("Banner Config", { KNOB: 1 });
H.assert(SC_get("Banner Config").description === "docs v2", "no declared description = no touch");

// --- v0.4.4: the longer log + the entry ceiling ---------------------------------------
for (let i = 1; i <= 25; i++) { global.info.actionCount = 400 + i; SC_report("Test", "line " + i); }
const logCard = SC_get("Event Log");
const evLines = logCard.entry.split("\n").filter(l => /^T\d+/.test(l));
H.assert(evLines.length === 20 && /line 25/.test(evLines[0]) && !/line 5\b/.test(logCard.entry),
    "window is 20, newest first, oldest rolled off");
global.info.actionCount = 430;
SC_report("Test", "X".repeat(120));
for (let i = 0; i < 8; i++) { global.info.actionCount = 431 + i; SC_report("Test", "Y".repeat(120)); }
H.assert(SC_get("Event Log").entry.length <= 990, "entry budget holds under long lines (ceiling outranks cap)");
H.assert(/YYYY/.test(SC_get("Event Log").entry.split("\n")[1]), "newest survives the trim; oldest paid for it");
