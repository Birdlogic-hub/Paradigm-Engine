...
| **InventoryKit/** *(né SlashInventory)* | **The Possession** — what the player holds, held honestly: bookkeeping deterministic, consequences adjudicated. Per-verb arbitration policy (none/outcome/gated). **v0.2: the Loadout** — equipment as possession with a worn flag. | SIS core; Cragin's policy idea | **v0.2.0** — harness-passed · **ESSENTIALS** |
| **BridgeKit/** *(né ISCompat)* | **The Truce** — third-party shim: the Check yields on guests' special turns (IS task, AC generation, LC thought, SAE control), and new characters get walked around the room (the Introduction). | Inner Self / Auto-Cards / Living Characters / SlowBurn compat studies | **v0.8.1** — AC yield LIVE-PROVEN ✔ · **PE CHARACTERS** |
| **SkillKit/** *(né RPG/ParaSkills)* | **The Skill** — the Check already names the skill behind every ruling (`skill=`); SkillKit remembers what you're good at. Ranks are semantic and reach the arbiter as words, never as `GK_setLuck`. **v0.2: the Growth** — Level derived from total contested accrual, plus attribute floors. | TAS's skill-learning (prior art); Dice's salvaged seam | **v0.2.1** — v0.1.x LIVE-PROVEN ✔ · **PRPG** |
...
| **EventKit/** *(né RandomEvents)* | **The Intrusion** — the world acts without player prompting: weighted category tables, flavor pools as data, effects attached by category. | RESR, Fusion Dungeon | **v0.1.1** — live-confirmed ✔ · **PRPG** |
| ~~Dice~~ | *Retired 7/13/2026* — the Check contains its own chance. Salvage in `Retired/Dice/` | Draftworlds d20 | retired |
| ~~Reputation~~ | *Retired 7/13/2026, pre-build* — ATM's system, never one of ours by design. No folder kept. | ATM | retired |
